...
interface Props {
  initialMode?: 'worlds' | 'skills' | 'myagent' | 'canvasLab';
...
type NetworkMode = 'worlds' | 'skills' | 'myagent' | 'canvasLab';
...
function NetworkHeader({ active, canvasSkillCount, onChange }: { active: NetworkMode; canvasSkillCount: number; onChange: (value: NetworkMode) => void }) {
  const title = active === 'skills' ? '我的技能' : active === 'canvasLab' ? '卡牌实验' : active === 'myagent' ? '我的智能体' : '智能体世界';
    ...
    <>
      ...
      <div className="shrink-0 border-b-2 border-black bg-white px-4 py-3.5">
        <div className="flex min-w-0 items-center justify-between gap-3">
          ...
          {active === 'canvasLab' && <span className="grid h-11 w-11 shrink-0 rotate-3 place-items-center border-2 border-black bg-[#ffd34e] shadow-[2px_2px_0_#000]"><Blocks className="h-6 w-6" strokeWidth={2.5} /></span>}
